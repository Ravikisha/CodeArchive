...
public:
    void helper(queue<int> &q, queue<int> &reversed)
    {
        if (q.empty())
        ...
        q.pop();
        helper(q, reversed);
        reversed.push(frontElement);
    }

    queue<int> reverseQueue(queue<int> &q)
    {
        queue<int> reversed; // Use a temporary queue for tail recursion
        helper(q, reversed); // Tail-recursive helper
        q = reversed;        // Copy the reversed queue back to the original
        return q;
    }
};
